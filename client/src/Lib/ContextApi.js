...
export const StateContext = ({children}) => {
    ...
    useEffect(() => {
        const bagData = JSON.parse(localStorage.getItem('bagItems'));
        if (bagData){
            setBagItems(bagData);
        }
    }, [])
    ...
    return (
        <Context.Provider value={{removeFromBag, bagQuantity,getItemsQuantity,decreaseBagQuantity,increaseBagQuantity,bagItems,}}>
            {children}
