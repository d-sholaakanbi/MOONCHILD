...
export const StateContext = ({ children }) => {
  ...
  const getItemQuantity = (id) => {
    return bagItems.find((item) => item.id === id.id)?.quantity || 0
  }

  const increaseBagQuantity = (id) => {
    setBagItems((currItems) => {
      if (currItems.find((item) => item.id === id.id) == null) {
  ...
  const decreaseBagQuantity = (id) => {
    setBagItems((currItems) => {
      if (currItems.find((item) => item.id === id.id)?.quantity === 1) {
        return currItems.filter((item) => item.id !== id.id)
      } else {
        return currItems.map((item) => {
          if (item.id === id.id) {
            return { ...item, quantity: item.quantity - 1 }
